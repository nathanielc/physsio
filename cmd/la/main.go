...
type Game struct {
	Teams [2]Team
	State [gameSize]float64
}

func (g *Game) teamA() Team {
	return team(g.State[:teamSize])
}
func (g *Game) teamB() Team {
	return team(g.State[teamSize+1:])
}
...
func (t *team) Name() string { return "" }
func (t *team) Active() Beast {
	active := int(t[0])
	b := 1 + beastSize*active
	e = b + beastSize
	return beast(t[b:e])
}
func (t *team) Beasts() []Beast {
	return []Beast{
		beast(t[1 : beastSize+1]),
		beast(t[1+beastSize : beastSize*2+1]),
		beast(t[1+beastSize*2:]),
	}
}
